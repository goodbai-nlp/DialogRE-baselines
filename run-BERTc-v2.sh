...
seed=42
setting=v2
#save_path=bert_f1_max-512-${model}-AMR-seed-${seed}-$setting
mkdir -p $save_path
if [ "$mode" == "train" ]
then
echo "Start Training..."
...
do
save_path=bert_f1_max-512-${model}-seed-${seed}-$setting
CUDA_VISIBLE_DEVICES=$dev python run.py --task_name bert --do_train --do_eval --do_evalc \
	--architecture $model \
	--seed $seed \
...
then
echo "Start Testing..."
CUDA_VISIBLE_DEVICES=$dev python run.py --task_name bert --do_eval --do_evalc \
	--architecture $model \
	--seed $seed \
	--vocab_file $BERT_BASE_DIR/vocab.txt \
	--bert_config_file $BERT_BASE_DIR/bert_config.json  \
